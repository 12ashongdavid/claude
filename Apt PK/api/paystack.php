...
if ($user['role'] !== 'tenant') {
    ...
    echo json_encode(['error' => 'Only tenants can make payments online.']);
    exit;
}
...
if ($type === 'rent') {
    ...
    if (!preg_match('/^\d{4}-\d{2}$/', $month)) {
        echo json_encode(['error' => 'Invalid billing month.']);
        exit;
    }
    if ($months < 1 || $months > 12) {
        echo json_encode(['error' => 'You can pay between 1 and 12 months of rent.']);
        exit;
    }
    $minMonth = date('Y-m');
    $maxMonth = date('Y-m', strtotime('+11 months'));
    if ($month < $minMonth) {
        echo json_encode(['error' => 'The paying-from month cannot be in the past.']);
        exit;
    }
    $endMonth = date('Y-m', strtotime($month . ' +' . ($months - 1) . ' months'));
    if ($endMonth > $maxMonth) {
        echo json_encode(['error' => 'Advance rent cannot exceed one year from now (up to ' . date('F Y', strtotime($maxMonth . '-01')) . ').']);
        exit;
    }
    ...
    if ($overlap) {
        echo json_encode(['error' => 'You have already paid rent for ' . implode(', ', $overlap) . '. Please select only months you have not paid for.']);
        exit;
    }
    ...
    $ten = $stmt->fetch();
    if (!$ten) {
        echo json_encode(['error' => 'No active tenancy found for your account.']);
        exit;
    }
    ...
    $bill = $stmt->fetch();
    if (!$bill) {
        echo json_encode(['error' => 'Bill not found or already paid.']);
        exit;
    }
...
if (!$res || !isset($res['status']) || $res['status'] !== true) {
    ...
    echo json_encode(['error' => 'Payment initialization failed: ' . $msg]);
    exit;
}
