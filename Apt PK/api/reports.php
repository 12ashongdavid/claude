...
$stmt = $db->prepare("SELECT COALESCE(SUM(rp.amount),0) AS revenue, COUNT(*) AS cnt FROM rent_payments rp WHERE 1=1 $where");
$stmt->execute($params);
$rent = $stmt->fetch();
...
$rentRows = [];
if (1) {
    $stmt = $db->prepare("SELECT rp.payment_date AS d, 'rent' AS kind, u.full_name AS tenant, r.room_number AS room, rp.reference_number AS ref, rp.payment_method AS method, rp.amount AS amount, rp.month_covered AS note FROM rent_payments rp JOIN users u ON rp.tenant_id = u.id JOIN rooms r ON rp.room_id = r.id WHERE 1=1 $where");
    $stmt->execute($params);
