        ...
        <div class="modal-body">
            <form id="confirmPaymentForm" onsubmit="submitConfirmPayment(event)">
                ...
                <div class="form-group">
                    <label style="font-weight:700;font-size:0.85rem;">Verification Code *</label>
                    <input type="text" name="verification_code" class="form-control" placeholder="Enter 6-digit code from booker" pattern="[0-9]{6}" maxlength="6" required style="font-size:1.1rem;letter-spacing:3px;text-align:center;">
                    <small style="color:var(--text-muted);font-size:0.75rem;">Ask the booker for the verification code they received via SMS.</small>
                </div>
                <div class="form-group">
        ...
        <div class="booking-card ${esc(b.status)}" style="border-left-color:var(--${statusColors[b.status] || 'info'});">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
                <div style="flex:1;min-width:200px;">
                    ...
                    <div style="margin-top:8px;padding:10px 14px;background:var(--surface);border-radius:8px;font-size:0.82rem;">
                        ...
                        ${b.verification_code && b.payment_status !== 'completed' ? '<br><strong style="color:var(--accent);">Verification Code:</strong> <code style="font-size:1rem;letter-spacing:2px;background:var(--accent-light);padding:2px 8px;border-radius:4px;">' + esc(b.verification_code) + '</code>' : ''}
                    </div>` : ''}
                </div>
            </div>
            <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
                ...
                ${hasPayment && b.payment_status !== 'completed' && b.payment_status !== 'failed' ? `
                    <button class="btn btn-sm btn-success" onclick="openConfirmPayment(${b.id}, '${esc(b.verification_code || '')}')"><i class="bx bx-check-circle"></i> Confirm Payment</button>
...
function openConfirmPayment(id, verificationCode) {
    document.getElementById('confirmPaymentBookingId').value = id;
    document.querySelector('input[name="payment_reference"]').value = '';
    document.querySelector('input[name="verification_code"]').value = verificationCode || '';
