...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCSRFToken($csrf)) {
        ...
        if (empty($full_name) || empty($phone) || empty($email)) {
            ...
            // Store SMS data for deferred sending (after page output)
            $pendingSms = [];
            if ($verification_code) {
                $booker_sms = "Dear $full_name, your booking request has been received" . ($payment_type !== 'none' ? " with a payment of " . formatCurrency($payment_amount) : "") . ". PK's Luxury Apartments will contact you within 24 hours. Your verification code is $verification_code. Please share this code with our team to confirm your payment.";
            } else {
...
<div id="reportSuccessModal" class="modal-overlay" onclick="if(event.target===this)closeReportSuccessModal()">
    <div style="background:#1B2A4A;border-radius:var(--radius-lg);box-shadow:var(--shadow-lg);max-width:460px;width:92%;padding:44px 32px 36px;text-align:center;position:relative;margin:20px;border:1px solid rgba(255,255,255,0.1);">
        <button onclick="closeReportSuccessModal()" style="position:absolute;top:12px;right:16px;background:none;border:none;font-size:1.5rem;cursor:pointer;color:rgba(255,255,255,0.5);line-height:1;" aria-label="Close">&times;</button>
