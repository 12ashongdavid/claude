...
$tenancy = null;
if (!$isAdmin) {
    $stmt = $db->prepare("SELECT t.*, r.apartment_number, r.rental_price, r.apartment_type, rt.charge_period FROM tenancies t JOIN apartments r ON t.apartment_id = r.id LEFT JOIN apartment_types rt ON rt.name = r.apartment_type WHERE t.tenant_id = ? AND t.status = 'active' ORDER BY t.id DESC LIMIT 1");
...
<div class="card" style="border-left:4px solid var(--accent);">
    <div class="card-header">
        ...
        <span class="text-muted" style="font-size:0.82rem;">Apartment <?= sanitize($tenancy['apartment_number']) ?> &bull; <?= ($tenancy['charge_period'] ?? 'monthly') === 'daily' ? 'Daily Rate' : 'Monthly Rate' ?></span>
    </div>
    <div class="card-body">
        <?php if ($fullyPaid): ?>
